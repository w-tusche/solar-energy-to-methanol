...
try:
    ...
    inp_file = impresources.files(data) / "ASTMG173.csv"
    # http://rredc.nrel.gov/solar/spectra/am1.5/
    ref_solar = pd.read_csv(inp_file, header=1)  # nm vs W m^-2 nm^-1
    # data range: 280nm to 4000nm, 0.31eV to 4.42857 eV
except:
    ref_solar = pd.read_csv(r"./data/raw/ASTMG173.csv", header=0, sep=";")

# File Import inside the module: https://stackoverflow.com/questions/6028000/how-to-read-a-static-file-from-inside-a-python-package
...
# data range: 280nm to 4000nm, 0.31eV to 4.42857

# Wavelength [nm]
WL = ref_solar.iloc[:, 0]

# Solar spectrum ASTMG173 direct:  [W*m-2*nm-1]
solar_per_nm = ref_solar.iloc[:, 2]

# Equal spacing for values inside the interval  (for interpolation)
Es = np.arange(start=0.32, stop=4.401, step=0.002)  # (equal spacing for interpolation)


class SQlim(object):
    def __init__(self, WL, solar_per_nm, T=300, EQE_EL=1.0, intensity=1.0):
        """
        T: temperature in K
        ...
        if T <= 0 or EQE_EL > 1.0 or EQE_EL <= 0:
            raise ValueError(
                "Both T and EQE_EL must be greater than 0 and "
                + "EQE_EL cannot be greater than 1.0"
            )

        self.WL = WL
        self.solar_per_nm = solar_per_nm
        self.T = T
        self.EQE_EL = EQE_EL
    ...
    def __spectrum_to_flux(self):
        """
        Calcualte a flux from a given spectrum Input WL (Wavelength in nm) and
        spectrum in nm as well.
        """
        # Electron volt data series [eV]
        self.E = 1240.0 / self.WL  # eV

        # jacobian transformation, W m^-2 eV^-1 (divide by photon energy)
        self.solar_per_E = self.solar_per_nm * (eV / 1e-9) * h * c / (eV * self.E) ** 2

        # linear interpolation to get an equally spaced spectrum
        self.AM15 = np.interp(
            x=self.Es, xp=self.E[::-1], fp=self.solar_per_E[::-1]
        )  # W m^-2 eV^-1
        self.AM15flux = self.AM15 / (self.Es * eV)  # number of photon m^-2 eV^-1 s^-1

        #### Flux in nm:
        self.WL_equally_spaced = np.arange(
            self.WL.min(), self.WL.max() + 0.1, 0.5
        )  # nm
        self.AM15_WL = np.interp(
            x=self.WL_equally_spaced, xp=self.WL, fp=self.solar_per_nm
        )  # # W m^-2 nm^-1
        # Divide by the photon energy (E = (h *c)/lambda) + to nm from m
        self.AM15flux_WL = (
            self.AM15_WL / (c * h / (self.WL_equally_spaced * 1e9)) / 1e9**2
        )  # number of photon m^-2 nm^-1 s^-1

        return None

    def __cal_E_Jsc(self):
        """
        Calcualte short-circuit current density (Jsc). Jsc = q*integrate(Photon
        flux)

        Args:
            flux (numpy.ndarray): number of photon [m^-2 eV^-1 s^-1]
            energy (numpy.ndarray): Energy interval to integrate over [eV]
            intensity (float): intensity (number of suns)

        Returns:
            Jsc: short-circuit current density [mA/cm2]
        """
        # Reversed since the cummulative needs to start at \infty and end always at the band gap E_g
        fluxcumm = cumulative_trapezoid(self.AM15flux[::-1], self.Es[::-1], initial=0)
        # Reverse again to have the fluxcumsum in the right direction
    ...
    def __cal_E_J0(self):
        """
        Calculate and return E vs J0, the dark saturation current J0 = q *
        (integrate(phi dE) from E to infinity)  / EQE_EL phi is the black body
        radiation at T (flux vs energy)

        Compare "absorption from the sun" in
        https://www.pveducation.org/pvcdrom/detailed-balance
        J0 = q * f0 * phi(E_g,T_c); f0 = 2 due to irradiance from front and back of planar cell (SQ) Formula can be found in Solar Cells Markvart and Castaner
        Jsc = q *fw * phi(E_g,T_s); fw = 2.18e-5 (geometric factor sun radiating onto earth)
        """
        Energy_in_joule = self.Es * eV
        ...
        phi = (
            ...
            * (
                ...
                * eV
                / ((h**3) * (c**2))
                / (np.exp(Energy_in_joule / (k * self.T)) - 1)
            )
        )

        # fluxcumm = sp.integrate.cumulative_trapezoid(phi[::-1], self.Es[::-1], initial=0)
        fluxcumm = cumulative_trapezoid(phi[::-1], self.Es[::-1], initial=0)
    ...
    def __cal_E_Voc(self):
        """
        Calculate and return E vs Voc Voc = (kT/q)*[ ln(Jsc/J0 + 1 )]

        (SQ-Paper 1961 formulas 1.2 and 3.18)

        Args:
            Jsc (_type_): short-circuit current density [mA/cm2]
            J0 (_type_): ??
            T (int, optional): Cell temperature [K]. Defaults to 300.

        Returns:
            _type_: Open-circuit voltage
        """
    ...
    def __cal_E_PCE(self):
        ...
        Returns:
           list: list of power conversions efficiencies per bandgap
        """
        PCE = []
        for i, E in enumerate(self.Es):
            V = np.arange(0, E, 0.001)  # can change upper bound to Voc
            # *-1 to allow minimum calculation
            J = -1 * self.Jsc[i] + self.J0[i] * (np.exp(q * V / (k * self.T)) - 1)
            # *-1 to make maximum out of minimum (MPP calculation)
            PCE.append(-1 * np.min(J * V) / self.intensity)
        return PCE

    def simulate_JV(self, Eg, plot_dark=False, Vstep=0.001, plot=True, Vmin=-0.5):
        """
        ...
        if plot:
            ...
            plt.ylabel(r"Current density (mA/$\mathregular{cm^2}$)", fontsize=16)
    ...
    def get_paras(self, Eg, toPrint=True):
        """
        Input Eg, return or print the corresponding parameters if toPrint ==
        True : print the result, return nothing if toPrint == False: return the
    ...
    def plot(self, para="PCE", xlims=(0.32, 4.5)):
        ...
        yunits = {
            "Voc": "(V)",
            "Jsc": r"(mA/$\mathregular{cm^2}$)",
            "FF": "(%)",
            "J0": r"(mA/$\mathregular{cm^2}$)",
            "PCE": "(%)",
        }
        plt.figure(para)
        ax = plt.gca()
    ...
    def plotall(self, xlims=(0.32, 3.0)):
        ...
        ylabel = ["Voc (V)", r"Jsc (mA/$\mathregular{cm^2}$)", "FF (%)", "PCE (%)"]
    ...
    def E_loss(self, Eg, xmin=300, xmax=2500, plot=False, savefig=False):
        """
        ...
        if plot:
            ...
            ax.set_ylabel(r"Irradiance  (W $\mathregular{m^{-2}\ nm^{-1}}$)", size=18)
    ...
    def available_E(
        ...
        """
        Plot the theoretical maximum available energies from a series of solar
        cells with different Egs. This is NOT the theoretical efficiencies for
        "two-terminal" tandem cells, which would requires current matching. The
        values here are for "mechanically stacked" tandem cells.
        ...
        WLs = self.WLs
        AM15nm = np.interp(WLs, self.WL, self.solar_per_nm)
        plt.figure(figsize=(8, 4.5))
        ax = plt.gca()
        # color options: darkgoldenrod, darkorange, yellow, black
        solarcolor = "gold"
        ...
        for n, Eg in enumerate(Egs[:-1]):
            ...
            if E_MPP:
                SQ_E = SQlim(
                    WL=self.WL,
                    solar_per_nm=self.solar_per_nm,
                    intensity=(Jscs[n] - Jscs[n + 1]) / Jscs[n],
                    T=self.T,
                    EQE_EL=self.EQE_EL,
                )
        ...
        if legend:
            ...
            if numEg == 1:
                ...
                labels = [
                    "Cell {0}, Eg={1:.2f} eV, PCE={2:.1f}%".format(
    ...
    # With SQlim. a class variable should be accessible!
    @classmethod
    def vary_temp(cls, T=(150, 200, 250, 300, 350), xlim=(0.3, 4.0), attr="PCE"):
        plt.figure()
        parameters = {"PCE", "Jsc", "FF", "Voc", "J0"}
        if attr not in parameters:
            print("Invalid attribute")
            return
        SQs = [SQlim(WL=WL, solar_per_nm=solar_per_nm, T=temp) for temp in sorted(T)]
        for SQ in SQs:
    ...
    @classmethod
    def vary_EQE_EL(cls, EQE_EL=(1e-6, 1e-4, 1e-2, 1), xlim=[0.3, 4.0], attr="PCE"):
        plt.figure()
        parameters = {"PCE", "Jsc", "FF", "Voc", "J0"}
        if attr not in parameters:
            print("Invalid attribute")
            return
        SQs = [
            SQlim(WL=WL, solar_per_nm=solar_per_nm, EQE_EL=EQE)
            for EQE in sorted(EQE_EL, reverse=True)
        ]
        for SQ in SQs:
    ...
    def vary_suns(cls, Suns=(1, 10, 100, 1000), xlim=[0.3, 4.0], attr="PCE"):
        ...
        if attr not in parameters:
            ...
            return
        SQs = [
            SQlim(WL=WL, solar_per_nm=solar_per_nm, intensity=sun)
    ...
    def __helper_plt(cls, xlim, attr="PCE"):
        ylabel = {
            ...
            "Voc": "Voc (V)",
            "FF": "FF (%)",
            "Jsc": r"Jsc (mA/$\mathregular{cm^2}$)",
            "J0": r"J0 (mA/$\mathregular{cm^2}$)",
        }
    ...
    def Js_tandem(cls, Es):
        ...
        P = [0] + [get_paras(E, toPrint=False)["Jsc"] for E in Es]
        df = pd.DataFrame()
        df["Bandgap (eV)"] = Es
...
if __name__ == "__main__":
    # plt.close('all')
    SQ = SQlim(WL=WL, solar_per_nm=solar_per_nm)
    SQ.plot()
    SQ.plotall()
    SQ.E_loss(1.337)
    SQ.available_E(1.6)
    plt.show()
    """
    # plot Voc, Jsc, FF, PCE SQ.plotall() # plot Voc, Jsc, FF, PCE in one
    figure with 4 subplots SQ.plot("Voc") # plot Voc only. SQ.plot("PCE") #
    plot Jsc...other valid inputs: "Jsc", "FF", "J0"

    # get Voc, Jsc, FF, PCE for a specific bandgap SQ.get_paras(1.337) #
    print out the values SQ.get_paras(1.337, toPrint=False) # get a
    dictionary of parameters

    # plot available energy and loss for a specific bandgap
    SQ.E_loss(1.3)

    # plot available energy for cells with many different bandgap
    materials #SQ.available_E([0.9, .6,1.5,1.2])

    ### calculate and plot JV curve SQ.simulate_JV(1.3, plot=True)
    """
